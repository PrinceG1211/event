...
function ShowImage() {
  ...
  const handleDelete = (id) => {
    fetch(Variables.apiURL + "Image/delete", {
      method: "POST",
      headers: {
        accept: "Application/json",
        "content-type": "Application/json",
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h4>Image Data</h4>
                  <Link to="/addImage" className="btn btn-outline-primary">Add</Link>
                </div>
                <div class="card-body">
                      ...
                      <tbody>
                        {
                          imageList.map((image, index) => (
                            <tr>
                              ...
                              <td>{image.path}</td>
                              <td>
                                <Link className="btn btn-icon icon-left btn-outline-info" to={`/editImage/${image.imageID}`}><i className="far fa-edit"></i>Edit</Link><br/><br/>
                                <button className="btn btn-icon icon-left btn-outline-danger" onClick={() => handleDelete(image.imageID)}><i className="fas fa-times"></i>Delete</button>
                              </td>
                            </tr>
                          ))
                        }
